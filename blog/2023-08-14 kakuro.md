...
All Kakuros I saw have a clue for _every_ row and column.
...
While that's great for some use-cases (such as generating Kakuros or converting them into the images in this article), they contain redundant information that gets in the way when trying to _solve_ the Kakuro.
...
Instead, we _gradually_ fill the cells one by one, and at each step, we are carefully paying attention so that the Kakuro remains valid.
...
Because we _know_ that we only fill in numbers between 1 and 9, we can remove the check for that.
...
Compared to the naive solver, this is _fast!_
It even solves the Wikipedia Kakuro in about 5 seconds.
To get a more comprehensive comparison between the solvers, I also attempted to solve some Kakuros from [kakuros.com](https://www.kakuros.com), their sizes ranging from 15x15 to 30x30.
...
There's a tradeoff: All the time you spend on deciding which cell to fill out is time you could have spent _actually_ filling out cells and trimming down the solution space.
...
Complexity-wise, `rust:HashSet`s are a good choice, but they pay for that with a _huge_ constant factor:
...
This might make sense if you store many items, but for _nine digits at most,_ a `rust:HashSet` is a huge beast you're unleashing on your code.
...
In the beginning, we can save what constraints each cell affects.
...
What we _can_ see is that the runtime is dominated by the allocation and freeing of memory:
...
I _could_ try to optimize the solver even further.
...
> The fastest one solves the Kakuro from the book in 89 ms, but runs out of memory on the 30x30 Kakuro.
