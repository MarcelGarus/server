...
![Conventional multi-switch vs. bus system](files/gira-knx-bus.png)
...
![image of HomeServer](files/gira-homeserver.jpg)
...
![Screenshot of the Gira HomeServer app](files/gira-official-app.jpg)
...
Here's a part of the setup communication – the app developers were really into the pipe character (`|`):
...
After this authentication process, the client can send a type `1` message with a device ID and a state.
...
For example, devices that you can toggle on or off have the `action.devices.traits.OnOff` trait.
...
![architecture of our setup](files/gira-home-architecture.png)
