...
* `93`: The client authenticates using the hash.
...
We have a few [Nest Home devices](https://store.google.com), so I thought it would be a fun exercise to integrate our devices in the Google Assistant so we can control them using voice.

Google Assistant has the concept of a [Home Graph](https://developers.google.com/assistant/smarthome/concepts/homegraph).
By registering as a smart appliance maker, I was able to make Google contact a little **Raspberry Pi server in our WiFi** every time we say something like "Hey Google, turn on the bathroom lights."
