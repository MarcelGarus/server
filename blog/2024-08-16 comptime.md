...
On x86`embed:_`64, it compiles the byte code to x86`embed:_`64 machine code.
...
On x86`embed:_`64, the Soil byte code is compiled to machine code.
Soil registers are directly mapped to x86`embed:_`64 registers – the `soil:a` register lives in `soil:r10`, the `soil:b` register in `soil:r11`, etc.
This way, most Soil byte code instructions map to a single x86`embed:_`64 machine instruction.
